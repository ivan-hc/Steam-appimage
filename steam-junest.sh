...
BINSAVED="mktemp md5sum cmp getopt zenity lspci steam"
...
_remove_more_bloatwares() {
	...
	bin_remove="gcc yay git systemd pacman mangoplot"
	for r in $bin_remove; do
		rm -Rf ./"$APP".AppDir/.junest/usr/bin/"$r"*
	done
	lib_remove="gcc libgo.so libgphobos.so libjavascript libwebkit perl guile systemd pacman/"
	for r in $lib_remove; do
		rm -Rf ./"$APP".AppDir/.junest/usr/lib/"$r"*
	done
	lib32_remove="gcc libgo.so libgphobos.so"
	for r in $lib_remove; do
		rm -Rf ./"$APP".AppDir/.junest/usr/lib32/"$r"*
	done
	share_remove="gcc ibus/dicts/emoji/ perl info autoconf automake git pacman help/  gir fonts/"
...
#APPNAME=$(cat ./"$APP".AppDir/*.desktop | grep 'Name=' | head -1 | cut -c 6- | sed 's/ /-/g' | tr '()' '')
APPNAME="Steam"
